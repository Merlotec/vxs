...
// Channel salts to decorrelate the three outputs
const SALT0 : u32 = 0x68bc21u;
const SALT1 : u32 = 0x2e5be3u;
const SALT2 : u32 = 0x09fb21du; // any distinct constants
...
// =================== 6D simplex core (vec3 out) ==============
fn snoise6_vec3(pos: vec3<f32>, seed: vec3<f32>, freq_scale: f32) -> vec3<f32> {
  ...
  var outv : vec3<f32> = vec3<f32>(0.0);
  ...
  for (var m: u32 = 0u; m <= 6u; m = m + 1u) {
    ...
    if (t0 > 0.0) {
      ...
      let g0 = grad6(h ^ SALT0);
      let g1 = grad6(h ^ SALT1);
      let g2 = grad6(h ^ SALT2);
      ...
      // Three independent channels, shared kernel and corners
      let s0 = dot(g0.ga, dA) + dot(g0.gb, dB);
      let s1 = dot(g1.ga, dA) + dot(g1.gb, dB);
      let s2 = dot(g2.ga, dA) + dot(g2.gb, dB);

      outv += w * vec3<f32>(s0, s1, s2);
    }
  }
...
// =================== fBm wrappers ==============================
fn simplex6_vec3_fbm(pos: vec3<f32>, seed: vec3<f32>) -> vec3<f32> {
  var f    : vec3<f32> = vec3<f32>(0.0);
  var amp  : f32 = 1.0;
  var freq : f32 = 1.0;
  for (var o: u32 = 0u; o < noise6.octaves; o = o + 1u) {
    f    += amp * snoise6_vec3(pos, seed, freq);
...
// Remap to roughly [0,1] per channel
fn simplex6_vec3_fbm01(pos: vec3<f32>, seed: vec3<f32>) -> vec3<f32> {
  return 0.5 + 0.5 * simplex6_vec3_fbm(pos, seed);
}
...
fn coord_noise(coord: vec3<f32>, view_pos: vec3<f32>) -> vec3<f32> {
    ...
    var err = dis * dis;
    return coord + simplex6_vec3_fbm01(coord, view_pos) * 0.4 * dis;            
}
